...
|---|---|---|---|---|---|
| T-001 | `api/pyproject.toml`, `uv.lock`, `.python-version`, ruff/mypy config, `app/main.py` (`/healthz` only), `app/settings.py` (incl. `ENVIRONMENT` flag gating `/docs`,`/redoc`,`/openapi.json` — BR-010) | ADR-003, ADR-010, AD-6, BR-010 | `uv sync --frozen` (uv 0.10.8 — GATE_6 Q3; `.python-version` 3.13 is installed by uv, the system Python is 3.11.9); `uv run ruff check .`; `uv run mypy app`; `curl :8000/healthz` → `{"status":"ok"}`; **local DB = installed PostgreSQL 17 service, not Docker** (GATE_6 Q1 — project-config.md `db_start`): `psql -U panchayat -d panchayat -c "select version()"` shows 17.x | — | S / low |
