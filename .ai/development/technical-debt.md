...
| TD-B15 | T-011 security review (2026-09-17) | `setCsrfToken` (`web/src/lib/csrf.ts`) has zero call sites — nothing in `api.ts`'s `request()` yet captures a `csrf_token` field from a successful response body, so ARCH-F4's rotation-after-login/logout/password-change requirement depends entirely on every future call site remembering to call it by hand. A missed call site produces a silent, hard-to-diagnose 403 loop. | frontend-developer at T-012 (login) / whichever task wires the `auth.ts` layer | when `src/lib/auth.ts` or the login/logout screens are built |
